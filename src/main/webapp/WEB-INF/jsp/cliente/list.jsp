...
<main class="container">
	...
	<div class="table-responsive">
		<table class="table table-bordered table-striped table-hover align-middle shadow-sm table-light">
			...
			<tbody>
			<c:forEach items="${cliente_list_attribute}" var="cliente">
				<tr>
					<td>${cliente.nome}</td>
					...
					<td>${cliente.attivo}</td>
						...
						<a class="btn btn-sm btn-outline-secondary" href="${pageContext.request.contextPath}/pizzeria/clienti/show/${cliente.id}" aria-label="Dettaglio">
						...
						<a class="btn btn-sm btn-outline-primary" href="${pageContext.request.contextPath}/pizzeria/clienti/edit/${cliente.id}" aria-label="Modifica">
							<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16" aria-hidden="true">
							...
							<c:choose>
								<c:when test="${cliente.attivo}">
									<form method="post" action="${pageContext.request.contextPath}/pizzeria/clienti/disattiva/${cliente.id}" class="d-inline">
										<button class="btn btn-sm btn-outline-danger" type="submit">Disattiva</button>
									</form>
								</c:when>
								<c:otherwise>
									<form method="post" action="${pageContext.request.contextPath}/pizzeria/clienti/attiva/${cliente.id}" class="d-inline">
